...
(async function boot(){
  ...
  const fbCfg = window.VAULLET_FIREBASE_CONFIG;
  if(!fbCfg || !fbCfg.apiKey){
    console.error('Firebase config not found. Make sure firebase_config.js is loaded.');
    document.getElementById('lock').innerHTML = `<div style="text-align:center; padding:40px;">
      <div class="modal-title display" style="color:var(--alert); margin-bottom:20px;">Configuration Error</div>
      <div class="help-text">Firebase credentials not found.<br>Check that firebase_config.js is properly loaded.</div>
    </div>`;
    return;
  }
  ...
  try{
    ...
    if(!cloudReady){
      throw new Error('Cloud initialization failed');
    }
  }catch(e){
    console.error('Firebase Auth initialization failed:', e);
    document.getElementById('lock').innerHTML = `<div style="text-align:center; padding:40px;">
      <div class="modal-title display" style="color:var(--alert); margin-bottom:20px;">Setup Error</div>
      <div class="help-text">Could not initialize Firebase.<br>${escapeHtml(e.message)}</div>
    </div>`;
    return;
  }
  
  // Wait a moment for Firebase Auth to be fully ready
